...
import { ArrowRight, Copy, ExternalLink, FolderOpen } from "lucide-react"
...
function getPayloadRepresentation(output: OutputDetail): {
  ...
  const payloadRepresentation = getNestedRecord(
    manifest ?? undefined,
    "payload_representation"
  )

  return {
    imagePayloadContract: getStringValue(
      payloadRepresentation?.image_payload_contract
    ),
...
export function buildOutputPreview(output: OutputDetail) {
  ...
  if (output.format === "parquet") {
    ...
    facts.push({
      label: "Row groups",
      value: String(
        getNumberValue(parquet?.row_group_count) ?? "Not available"
      ),
    })
    facts.push({
      label: "Schema fields",
      value:
        schemaFields.length > 0 ? String(schemaFields.length) : "Not available",
    })
  ...
  if (notes.length === 0) {
    notes.push(
      "This preview is built from the backend output artifact metadata and manifest summaries."
    )
  }
      ...
      <CardContent className="space-y-5">
        ...
        <div className="space-y-2">
          ...
          {preview.notes.map((note) => (
            <div
              className="rounded-lg border bg-muted/10 px-3 py-3 text-sm text-foreground"
              key={note}
            >
              {note}
            </div>
...
export function OutputDetailContent({
  ...
  onRevealFilePath: (output: OutputDetail) => Promise<void>
  output: OutputDetail
}) {
      ...
      <Card>
        ...
        <CardContent className="space-y-5">
          <dl className="grid gap-4 sm:grid-cols-2">
            <MetadataField
              label="Format"
              value={formatOutputFormat(output.format)}
            />
            <MetadataField
              label="Size"
              value={formatFileSize(output.size_bytes)}
            ...
            <MetadataField
              label="Updated"
              value={formatDateTime(output.updated_at)}
            />
            <MetadataField
              label="Media type"
              value={output.media_type ?? "Not available"}
                ...
                {localFilePath ? (
                  <div className="flex items-start gap-2">
                    <button
                      ...
                      onClick={() => void onRevealFilePath(output)}
                    ...
                    <Button
                      ...
                      onClick={() => void onCopyFilePath(localFilePath)}
                      size="icon-sm"
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {output.asset_ids.length === 1 ? (
              <Button asChild size="sm" variant="outline">
                <AppLink
                  href={buildAssetDetailHref(output.asset_ids[0], currentHref)}
                >
                  Open asset
                  <ArrowRight className="size-3.5" />
                </AppLink>
      ...
      {Object.keys(output.metadata).length > 0 ? (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Backend-supplied metadata, manifest summaries, and artifact
              inspection results.
            </CardDescription>
          </CardHeader>
          <CardContent>
